...
| Section | |
|---------|--|
| [1. Why explore HypercubeWTF](#1-why-explore-hypercubewtf) | Role, benefits, application scope |
| [2. The big picture](#2-the-big-picture) | Where WTF sits among ESN / CNN |
| [3. One episode](#3-one-episode-step-by-step) | Dynamics, spatial→temporal, mechanics |
...
HypercubeWTF also processes **spatial data sets**, however it differs from
HypercubeCNN in that before reaching the convolution engine, each data set is
passed through a graph dynamical encoder (the reservoir). The convolution
engine never sees the original data set; it sees an encoded version of it
generated by the reservoir.

The internal dynamics of that particular encoding process (as opposed to more traditional static function encoders) appear to have some interesting
transformational properties that we are just beginning to explore — for instance,
filtering white noise when present, while acting as an identity transformation
when noise is not present, and reducing sensitivity to training data quality when
noise is present. For the details, see
...
The question of "is there real value" in this dynamic encoding - cnn pipeline has not yet been answered.
...
In short: **spatial pattern in → synthetic time by re-addressing → reservoir
dynamics → one end snapshot → trained head.** The recurrent core is not learned;
only the head is.
...
`collect_threads` for faster bulk collect.
...
`episode.bypass_reservoir` (requires `B == 1`) copies the field into the feature
buffer with no orbit. Measurement control for tests and studies only — default
off. Collect noise still applies when σ > 0.
...
|------|-----------------|
| `tests/wtf_smoke.cpp` | Contracts: sizes, determinism, parallel collect, noise |
...
| Construct fails with `bypass_reservoir` | Needs `B == 1` (eval control) |
